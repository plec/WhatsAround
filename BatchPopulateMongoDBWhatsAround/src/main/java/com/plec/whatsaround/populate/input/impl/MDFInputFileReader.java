...
public class MDFInputFileReader implements InputFileReader {

	private static final Logger LOGGER = Logger.getLogger(MDFInputFileReader.class);
	...
	private POI convert(String[] splittedLine) {
		POI poi = new POI();
		try {
			poi.setType(POIType.Musee);
			poi.setSourceId(splittedLine[0]);
			poi.setName(cleanString(splittedLine[6]));
			poi.setAdresse(cleanString(splittedLine[7]));
			poi.setCp(cleanString(splittedLine[8]));
			poi.setVille(cleanString(splittedLine[9]));
			poi.setDescription(cleanString(splittedLine[10]));
			poi.setPeriodeFermeture(cleanString(splittedLine[11]));
			poi.setHoraires(cleanString(splittedLine[12]));
			return poi;
